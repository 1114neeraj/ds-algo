 ...
 *
 */
public class BinarySearchOnMatrixStrategy implements PlayerStrategy{

	private BlockingQueue<Message> producerChannel;
	private BlockingQueue<Message> consumerChannel;
	private Scorer scorer;

	public BinarySearchOnMatrixStrategy(BlockingQueue<Message> producerChannel,
...
//		print(matrix);
	...
	private int solve(int[][] matrix, int r1, int c1, int r2, int c2, int totalCount) throws InterruptedException {
		...
		if(r1==r2) {
			return solveRow(matrix, r1, c1, c2, totalCount);
		}
...
//			System.out.println(String.format("%s %s %s %s %s", 1, r1, c1, r2, c2));
		...
		int topHalfCount = solve(matrix, r1, c1, r3, c2, -1);
		...
		if(bottomHalfCount > 0) {
			bottomHalfCount = solve(matrix, r3+1, c1, r2, c2, bottomHalfCount);
		}
